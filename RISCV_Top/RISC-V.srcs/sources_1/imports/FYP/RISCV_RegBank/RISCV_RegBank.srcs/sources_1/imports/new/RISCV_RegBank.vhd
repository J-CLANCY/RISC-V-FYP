...
begin
    if rs1_addr = "00000" then
        ...
        rs1_data <= reg_bank(to_integer(unsigned(rs1_addr)));
    end if;
end process;
...
begin
    if unsigned(rs2_addr) = "00000" then
        ...
        rs2_data <= reg_bank(to_integer(unsigned(rs2_addr)));
    end if;
end process;
